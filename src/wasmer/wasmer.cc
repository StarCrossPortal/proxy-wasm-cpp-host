...
bool Wasmer::load(std::string_view bytecode, std::string_view,
...
bool Wasmer::link(std::string_view debug_name) {
...
uint64_t Wasmer::getMemorySize() { return wasm_memory_data_size(memory_.get()); }

std::optional<std::string_view> Wasmer::getMemory(uint64_t pointer, uint64_t size) {
  assert(memory_ != nullptr);
...
bool Wasmer::setMemory(uint64_t pointer, uint64_t size, const void *data) {
  assert(memory_ != nullptr);
  if (pointer + size > wasm_memory_data_size(memory_.get())) {
    return false;
  }
...
bool Wasmer::getWord(uint64_t pointer, Word *word) {
  assert(memory_ != nullptr);
...
bool Wasmer::setWord(uint64_t pointer, Word word) {
  constexpr auto size = sizeof(uint32_t);
  if (pointer + size > wasm_memory_data_size(memory_.get())) {
    return false;
  }
...
template <typename... Args>
void Wasmer::registerHostFunctionImpl(std::string_view module_name,
...
template <typename... Args>
void Wasmer::getModuleFunctionImpl(std::string_view function_name,
                                     std::function<void(ContextBase *, Args...)> *function) {
...
std::unique_ptr<WasmVm> createWasmerVm() { return std::make_unique<wasmer::Wasmer>(); }
